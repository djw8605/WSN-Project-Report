...
\subsection{Simulation Generator}
...
transfer_input_files = \
   output2/submit0.simconfig, \
   simulator.tar.gz
log = condor.log
when_to_transfer_output = ON_EXIT
universe = vanilla
arguments = output2/submit0.simconfig
error = output2/submit0.simconfig.err
output = output2/submit0.simconfig.out
queue
\end{lstlisting}
...
When the job is submitted to Condor, it will manage the execution of the job until completion.  First it will send a request to the central manager for nodes to run on.  After receiving confirmation of a available node, it will contact the worker node and begin transferring files.  The transferred files are those specified by the \texttt{transfer_input_files}, plus the executable.  Once the transfers are completed, Condor will begin the executable.  

The executable is custom for our project, and is distributed with the source.  It will first uncompress the NS-2 simulator, then execute the simulation using the simulation configuration that was given to the the executable as the \texttt{arguments}.
